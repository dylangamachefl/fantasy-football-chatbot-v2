...
# --- UPDATED SECTION ---
# Import the V2 app and state initializer
from graph_builder_v2 import app
from agent_state_v2 import create_initial_state


def run_agent_on_question(question: str):
    """
    A simplified, non-Streamlit function to run the V2 AGENT GRAPH
    for a single question.
    """
    rate_limit_callback = RateLimitingCallbackHandler(delay_seconds=5)

    # Use the create_initial_state helper from agent_state_v2.py
    # This correctly builds the AgentState dictionary with all required
    # default fields (e.g., conversation_history=[], iteration_count=0, etc.)
    input_state = create_initial_state(question)

    try:
        # We invoke the entire graph with the initial state
        response = app.invoke(
            input_state,
            # We pass our callbacks here for LangSmith tracing
            config={"callbacks": [rate_limit_callback]},
        )
        # The final answer is in 'final_answer' from our AgentState
...
def main():
    ...
    """
    logger.info("--- Starting V2 Agent Evaluation ---")
    ...
    if total_questions > 0:
        ...
        logger.info("\n" + "=" * 40)
        logger.info("           EVALUATION SUMMARY (V2)")
        logger.info("=" * 40)
